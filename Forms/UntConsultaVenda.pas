unit UntConsultaVenda;

interface

uses
  Winapi.Windows,
  Winapi.Messages,

  System.SysUtils,
  System.Variants,
  System.Classes,

  FireDAC.Stan.Param,

  UntBase,
  UntComponentesGerais,
  UntDataModule,
  UntConsultaCliente,
  UntBibliotecaFuncoes,

  Data.DB,

  Vcl.Graphics,
  Vcl.Controls,
  Vcl.Forms,
  Vcl.Dialogs,
  Vcl.StdCtrls,
  Vcl.Grids,
  Vcl.DBGrids,
  Vcl.ComCtrls;

type
  TFrmConsultaVenda = class(TFrmBase)
    GroupBoxHeader: TGroupBox;
    GroupBoxFooter: TGroupBox;
    GroupBoxGrid: TGroupBox;
    DBGridVenda: TDBGrid;
    EditCodigoCliente: TEdit;
    ButtonConsultar: TButton;
    ButtonFechar: TButton;
    CheckBoxFinalizadas: TCheckBox;
    DateTimePickerInicial: TDateTimePicker;
    DateTimePickerFinal: TDateTimePicker;
    Label1: TLabel;
    Label2: TLabel;
    Label3: TLabel;
    ButtonConsultaCliente: TButton;
    ButtonEstornarVenda: TButton;
    LabelNomeCliente: TLabel;
    procedure ButtonConsultarClick(Sender: TObject);
    procedure FormShow(Sender: TObject);
    procedure FormClose(Sender: TObject; var Action: TCloseAction);
    procedure ButtonConsultaClienteClick(Sender: TObject);
    procedure EditCodigoClienteExit(Sender: TObject);
    procedure ButtonFecharClick(Sender: TObject);
    procedure ButtonEstornarVendaClick(Sender: TObject);
  private
    function EstornaCaixa(): Boolean;
    function EstornaFinanceiro(): Boolean;
    function EstornaProdutos(): Boolean;
    procedure EstornaVenda;
    procedure FecharQuerys;
    { Private declarations }
  public
    { Public declarations }
  end;

var
  FrmConsultaVenda: TFrmConsultaVenda;

implementation

{$R *.dfm}

procedure TFrmConsultaVenda.ButtonEstornarVendaClick(Sender: TObject);
begin
  inherited;
  try
    try
      ButtonEstornarVenda.Enabled := False;

      // Verifica se tem algo pesquisado
      if DM.FDQConsultaVenda.Eof then
      begin
        Mensagem(2,'Deve-se consultar uma venda');
        EditCodigoCliente.SetFocus;
        Exit;
      end;

      // Verifica se a venda selecionada esta finalizada
      if DM.FDQConsultaVenda.FieldByName('STATUS').AsString = 'G' then
      begin
        Mensagem(2,'Venda deve estar finalizada');
        EditCodigoCliente.SetFocus;
        Exit;
      end;

      // Verifica onde foi lançado o financeiro
      DM.FDQConsulta.Close;
      DM.FDQConsulta.SQL.Text := 'SELECT * FROM FORMAPAGTO WHERE CODIGO = :CODIGO';
      DM.FDQConsulta.Params.ParamByName('CODIGO').AsInteger := DM.FDQConsultaVenda.FieldByName('FORMAPAGTO').AsInteger;
      DM.FDQConsulta.Open;

      if DM.FDQConsulta.FieldByName('LANCACAIXA').AsBoolean then
      begin
        if EstornaCaixa then
          EstornaVenda;
      end
      else if DM.FDQConsulta.FieldByName('LANCAFINANCEIRO').AsBoolean then
      begin
        if EstornaFinanceiro then
          EstornaVenda;
      end
      else if DM.FDQConsulta.FieldByName('LANCAPARCELADO').AsBoolean then
      begin
        if EstornaFinanceiro then
          EstornaVenda;
      end
      else if DM.FDQConsulta.FieldByName('LANCASEMCONTROLE').AsBoolean then
      begin
        EstornaVenda;
      end;

      DM.FDQConsulta.Close;

      ButtonConsultarClick(Sender);

    finally
      ButtonEstornarVenda.Enabled := True;
    end;
  Except on E:Exception do
    Mensagem(3,E.ToString);
  end;
end;

procedure TFrmConsultaVenda.ButtonConsultaClienteClick(Sender: TObject);
begin
  inherited;
  try
    try
      LabelNomeCliente.Caption := '';
      FrmConsultaCliente.ShowModal;
    finally
      if not (DM.FDQCliente.State in [dsInactive]) then
      begin
        EditCodigoCliente.Text := DM.FDQCliente.FieldByName('CODIGO').AsString;
        LabelNomeCliente.Caption := DM.FDQCliente.FieldByName('NOME').AsString;
        EditCodigoCliente.SetFocus;
      end;
      DM.FDQCliente.Close;
    end;
  Except on E:Exception do
    Mensagem(3,E.ToString);
  end;
end;

procedure TFrmConsultaVenda.ButtonConsultarClick(Sender: TObject);
begin
  inherited;
  try
    try
      ButtonEstornarVenda.Enabled := False;

      DM.FDQConsultaVenda.close;

      DM.FDQConsultaVenda.SQL.Text := 'SELECT A.*, B.Nome as NomeCliente, C.Descricao as DescriFormaPagto ' +
      'FROM CABEVENDA as A ' +
      'JOIN CLIENTE as B on A.cliente = B.codigo ' +
      'JOIN FORMAPAGTO as C on A.formapagto = C.codigo ' +
      'WHERE A.DATA BETWEEN :DATAINICIAL AND :DATAFINAL AND A.STATUS = :STATUS ';

      DM.FDQConsultaVenda.Params.ParamByName('DATAINICIAL').AsDate := DateTimePickerInicial.Date;
      DM.FDQConsultaVenda.Params.ParamByName('DATAFINAL').AsDate := DateTimePickerFinal.Date;
      if CheckBoxFinalizadas.Checked then
        DM.FDQConsultaVenda.Params.ParamByName('STATUS').AsString := 'F'
      else
        DM.FDQConsultaVenda.Params.ParamByName('STATUS').AsString := 'G';

      // Se tiver o cliente ele procura por cliente
      if not (EditCodigoCliente.Text = EmptyStr) then
        DM.FDQConsultaVenda.SQL.Text := DM.FDQConsultaVenda.SQL.Text + 'AND CLIENTE = ' + EditCodigoCliente.Text;

      // Ordena Pela Data
      DM.FDQConsultaVenda.SQL.Text := DM.FDQConsultaVenda.SQL.Text + ' ORDER BY DATA, CODIGO';

      DM.FDQConsultaVenda.Open;

      DBGridVenda.Refresh;
      DBGridVenda.SetFocus;

      if CheckBoxFinalizadas.Checked then
        ButtonEstornarVenda.Enabled := True;
    finally

    end;
  Except on E:Exception do
    Mensagem(3,E.ToString);
  end;
end;


procedure TFrmConsultaVenda.ButtonFecharClick(Sender: TObject);
begin
  inherited;
  try
    try
      FrmConsultaVenda.Close;
    finally

    end;
  Except on E:Exception do
    Mensagem(3,E.ToString);
  end;
end;

procedure TFrmConsultaVenda.EditCodigoClienteExit(Sender: TObject);
begin
  inherited;
  try
    try
      LabelNomeCliente.Caption := '';

      // Se tiver colocado algum codigo ele pesquisa o cliente
      if not (EditCodigoCliente.Text = EmptyStr) then
      begin
        DM.FDQCliente.Close;
        DM.FDQCliente.SQL.Text := 'SELECT * FROM CLIENTE WHERE CODIGO = :CODIGO';
        DM.FDQCliente.Params.ParamByName('CODIGO').AsString := EditCodigoCliente.Text;
        DM.FDQCliente.Open;

        if DM.FDQCliente.Eof then
        begin
          Mensagem(1,'Cliente Não Encontrado');
          EditCodigoCliente.Clear;
          ButtonConsultaCliente.SetFocus;
          Exit;
        end
        else
        begin
          LabelNomeCliente.Caption := DM.FDQCliente.FieldByName('NOME').AsString;
          ButtonConsultar.SetFocus;
        end;
      end;
    finally
      DM.FDQCliente.Close;
    end;
  Except on E:Exception do
    Mensagem(3,E.ToString);
  end;
end;

function TFrmConsultaVenda.EstornaCaixa(): Boolean;
begin
  Result := False;
  try
    try
      DM.FDQCaixa.Close;
      DM.FDQCaixa.SQL.Text := 'SELECT * FROM CAIXA WHERE DOCUMENTO = :DOCUMENTO';
      DM.FDQCaixa.Params.ParamByName('DOCUMENTO').AsString := '2000' + DM.FDQConsultaVenda.FieldByName('CODIGO').AsString;
      DM.FDQCaixa.Open;

      // Verifica se encontrou o registro
      if DM.FDQCaixa.Eof then
      begin
        Mensagem(2,'Registro não encontrado no caixa');
        Exit;
      end
      else
      begin
        DM.FDQCaixa.Delete;
        Result := True;
      end;

      DM.FDQCaixa.Close;
    finally

    end;
  Except on E:Exception do
    Mensagem(3,E.ToString);
  end;
end;

function TFrmConsultaVenda.EstornaFinanceiro(): Boolean;
begin
  Result := False;
  try
    try
       // Verifica o recebido
      DM.FDQContaRecebida.Close;
      DM.FDQContaRecebida.SQL.Text := 'SELECT * FROM CONTARECEBIDA WHERE DOCUMENTO = :DOCUMENTO';
      DM.FDQContaRecebida.Params.ParamByName('DOCUMENTO').AsString := '2000' + DM.FDQConsultaVenda.FieldByName('CODIGO').AsString;
      DM.FDQContaRecebida.Open;

      if not DM.FDQContaRecebida.Eof then
      begin
        Mensagem(2,'Venda com conta já recebida.' + sLineBreak + 'Favor estornar as contas recebidas antes de estornar a venda' + sLineBreak +  'Numero do documento: 2000' + DM.FDQConsultaVenda.FieldByName('CODIGO').AsString);
        Exit;
      end
      else
      begin
        DM.FDQContaReceber.Close;
        DM.FDQContaReceber.SQL.Text := 'SELECT * FROM CONTARECEBER WHERE DOCUMENTO = :DOCUMENTO';
        DM.FDQContaReceber.Params.ParamByName('DOCUMENTO').AsString := '2000' + DM.FDQConsultaVenda.FieldByName('CODIGO').AsString;
        DM.FDQContaReceber.Open;

        DM.FDQContaReceber.First;
        while not DM.FDQContaReceber.Eof do
        begin
          DM.FDQContaReceber.Delete;
          //DM.FDQContaReceber.Next;
        end;

        Result := True;
      end;

      DM.FDQContaRecebida.Close;
      DM.FDQContaReceber.Close;
    finally

    end;
  Except on E:Exception do
    Mensagem(3,E.ToString);
  end;
end;

function TFrmConsultaVenda.EstornaProdutos: Boolean;
begin
  Result := False;
  try
    try
      // Se estiver marcado para baixar o estoque entao vai estornar o estoque tambem
      if DM.FDQParametros.FieldByName('BAIXAESTOQUEDAVENDA').AsBoolean then
      begin
        // Pegar os produtos da venda
        DM.FDQVenda.Close;
        DM.FDQVenda.SQL.Text := 'SELECT * FROM VENDA WHERE CABEVENDA = :CABEVENDA ';
        DM.FDQVenda.Params.ParamByName('CABEVENDA').AsInteger := DM.FDQCabeVenda.FieldByName('CODIGO').AsInteger;
        DM.FDQVenda.Open;

        if DM.FDQVenda.Eof then
        begin
          Mensagem(2,'Não foram encontrados os produtos da venda');
          Result := False;
          Exit;
        end
        else
        begin
          DM.FDQVenda.First;

          while not DM.FDQVenda.Eof do
          begin
            // Vai procurar e voltar o estoque de cada um dos produtos
            DM.FDQProduto.Close;
            DM.FDQProduto.SQL.Text := 'SELECT * FROM PRODUTO WHERE CODIGO = :CODIGO';
            DM.FDQProduto.Params.ParamByName('CODIGO').AsInteger := DM.FDQVenda.FieldByName('PRODUTO').AsInteger;
            DM.FDQProduto.Open;

            if not DM.FDQProduto.Eof then
            begin
              DM.FDQProduto.Edit;
              DM.FDQProduto.FieldByName('ESTOQUE').AsFloat := DM.FDQProduto.FieldByName('ESTOQUE').AsFloat + DM.FDQVenda.FieldByName('QUANTIDADE').AsFloat;
              DM.FDQProduto.Post;
            end;

            DM.FDQVenda.Next;
          end;

          Result := True;

        end;
      end
      else
      begin
        Result := True;
      end;
    finally

    end;
  Except on E:Exception do
    Mensagem(3,E.ToString);
  end;
end;

procedure TFrmConsultaVenda.EstornaVenda;
begin
  try
    try
      // Extorna a venda
      DM.FDQCabeVenda.Close;
      DM.FDQCabeVenda.SQL.Text := 'SELECT * FROM CABEVENDA WHERE CODIGO = :CODIGO';
      DM.FDQCabeVenda.Params.ParamByName('CODIGO').AsInteger := DM.FDQConsultaVenda.FieldByName('CODIGO').AsInteger;
      DM.FDQCabeVenda.Open;

      // Se achou a venda ele extorna ela
      if DM.FDQCabeVenda.Eof then
      begin
        Mensagem(2,'Venda não encontrada');
        Exit;
      end
      else
      begin
        // Altera o status do serviço apenas se estornou os produtos
        if EstornaProdutos then
        begin
          DM.FDQCabeVenda.Edit;
          DM.FDQCabeVenda.FieldByName('STATUS').AsString := 'G';
          DM.FDQCabeVenda.Post;
          Mensagem(1,'Venda Estornada com Sucesso');
        end;
      end;
    finally

    end;
  Except on E:Exception do
    Mensagem(3,E.ToString);
  end;
end;

procedure TFrmConsultaVenda.FecharQuerys;
begin
  try
    try
      DM.FDQProduto.Close;
      DM.FDQCliente.Close;
      DM.FDQConsulta.Close;
      DM.FDQCaixa.Close;
      DM.FDQContaReceber.Close;
      DM.FDQContaRecebida.Close;
    finally

    end;
  Except on E:Exception do
    Mensagem(3,E.ToString);
  end;
end;

procedure TFrmConsultaVenda.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  inherited;
  try
    try
      FecharQuerys;
      EditCodigoCliente.Clear;
      EditCodigoCliente.SetFocus;

      // Se tiver Pesquisado
      if DM.FDQConsultaVenda.State in [dsBrowse] then
      begin
        DM.FDQCabeVenda.Close;
        DM.FDQCabeVenda.SQL.Text := 'SELECT * FROM CABEVENDA WHERE CODIGO = :CODIGO ';
        DM.FDQCabeVenda.Params.ParamByName('CODIGO').AsInteger := DM.FDQConsultaVenda.FieldByName('CODIGO').AsInteger;
        DM.FDQCabeVenda.Open;

        DM.FDQVenda.Close;
        DM.FDQVenda.SQL.Text := 'SELECT * FROM VENDA WHERE CABEVENDA = :CABEVENDA ORDER BY NUMEROITEM ';
        DM.FDQVenda.Params.ParamByName('CABEVENDA').AsInteger := DM.FDQConsultaVenda.FieldByName('CODIGO').AsInteger;
        DM.FDQVenda.Open;
      end;
    finally

    end;
  Except on E:Exception do
    Mensagem(3,E.ToString);
  end;
end;

procedure TFrmConsultaVenda.FormShow(Sender: TObject);
begin
  inherited;
  try
    try
      LabelNomeCliente.Caption := '';
      FecharQuerys;
      DM.FDQConsultaVenda.Close;
      EditCodigoCliente.Clear;
      EditCodigoCliente.SetFocus;

      CheckBoxFinalizadas.Checked := False;
      ButtonEstornarVenda.Enabled := False;

      // Define as datas para hoje
      DateTimePickerInicial.Date := Date;
      DateTimePickerFinal.Date := Date;

    finally

    end;
  Except on E:Exception do
    Mensagem(3,E.ToString);
  end;
end;

end.
